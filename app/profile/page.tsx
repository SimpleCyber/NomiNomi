...
export default function ProfilePage() {
  ...
  const fetchWalletBalances = async () => {
    ...
    try {
      ...
      const tokens = Array.from(tokensMap.entries()).map(
        ([assetId, amount]) => {
  ...
  useEffect(() => {
    if (
      isConnected &&
      (activeTab === "Balances" || activeTab === "Coin Held") &&
      !balancesFetched &&
      !balancesLoading
    ) {
      fetchWalletBalances();
  ...
  const isCryptoView = ["Balances", "Coin Created", "Coin Held"].includes(
    activeTab,
  );
                        ...
                        <thead>
                          <tr className="text-left text-xs text-[var(--muted)] border-b border-[var(--border-color)]">
                            ...
                            {activeTab === "Coin Created" ? (
                              <>
                                <th className="p-4 font-medium text-right uppercase">
                                  Market Cap
                                </th>
                                <th className="p-4 font-medium text-right uppercase">
                                  Age
                            ...
                            <th className="p-4 font-medium text-right uppercase pr-6">
                              Change (24h)
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {currentData.map((item: any) => (
                            ...
                            >
                              ...
                              {activeTab === "Coin Created" ? (
                                <>
                                  <td className="text-right text-sm text-[var(--foreground)] p-4">
                                    {item.marketCap || "-"}
                                  </td>
                                  <td className="text-right text-sm text-[var(--foreground)] p-4">
                                    {item.age || "-"}
                              ...
                              <td
                                className={`text-right text-sm font-medium p-4 pr-6 ${item.change?.startsWith("+") ? "text-green-400" : "text-red-400"}`}
                              >
                                {item.change || "0%"}
                              </td>
                            </tr>
                    ...
                    {currentData.map((item: any) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          ...
                          <div
                            className={`flex items-center gap-1 text-sm font-bold ${item.change?.startsWith("+") ? "text-green-400" : "text-red-400"}`}
                          >
