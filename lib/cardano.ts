...
export const initLucid = async (walletApi?: any) => {
  ...
  // Auto-detect network based on key prefix
  ...
  try {
    ...
    if (error.message.includes("BigInt")) {
       throw new Error(`Network Connection Failed: Possible mismatch. Detected '${network}' from key, but initialization failed. Check your key.`);
    }
    throw error;
  }
};
