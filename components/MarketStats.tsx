      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex-1">
            <div className="font-bold text-[var(--foreground)]">{coin.name}</div>
        ...
        <div className="flex flex-col gap-2 min-w-[120px]">
          ...
          <div>
            <div className="flex justify-between text-xs text-[var(--muted)] mb-1">
              <span>Bonding</span>
              <span className={bondingCurve > 80 ? "text-amber-400" : "text-emerald-400"}>
                {bondingCurve}%
              </span>
            </div>
          ...
          <div className="flex justify-between text-xs">
            ...
            <span className="font-medium text-[var(--foreground)]">{holders}</span>
          </div>
        </div>
      </div>
    </div>
...
export default function MarketStats() {
  ...
  const getRandomUniqueCoin = (excludeIds: Set<string>): Coin | null => {
    ...
    if (availableCoins.length === 0) {
      ...
      return allCoins.length > 0 ? allCoins[Math.floor(Math.random() * allCoins.length)] : null;
    }
