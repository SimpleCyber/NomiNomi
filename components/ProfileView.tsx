...
import { Copy, ExternalLink, Loader2, MessageCircle, UserPlus, UserMinus } from "lucide-react";
...
export default function ProfileView({ walletAddress, initialUserData, isOwner = false }: ProfileViewProps) {
    const router = useRouter();
    const { walletAddress: currentUserAddress } = useWallet();
    const [activeTab, setActiveTab] = useState("Balances");
    const [userData, setUserData] = useState<UserProfile | null>(initialUserData || null);
    ...
    const handleFollowToggle = async () => {
        ...
        try {
            if (isFollowingUser) {
                ...
                if (userData) {
                    setUserData({ ...userData, followers: Math.max(userData.followers - 1, 0) });
                }
            } else {
                    ...
                    {!isOwner && (
                        <>
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium transition-colors border ${isFollowingUser
